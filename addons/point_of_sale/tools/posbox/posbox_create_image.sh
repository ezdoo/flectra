...
if [ ! -d $CLONE_DIR ]; then
    ...
    mkdir -p "${CLONE_DIR}"
    git clone -b 8.0 --no-local --no-checkout --depth 1 https://github.com/flectra/flectra.git "${CLONE_DIR}"
    cd "${CLONE_DIR}"
...
openerp/
flectra.py" | tee --append .git/info/sparse-checkout > /dev/null
...
QEMU_OPTS=(-kernel kernel-qemu -cpu arm1176 -m 256 -M versatilepb -no-reboot -serial stdio -append 'root=/dev/sda2 rootfstype=ext4 rw' -hda posbox.img -net user,hostfwd=tcp::10022-:22,hostfwd=tcp::17073-:7073 -net nic)
if [ -z ${DISPLAY:-} ] ; then
    QEMU_OPTS+=(-nographic)
fi
