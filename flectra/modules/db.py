...
def is_initialized(cr):
    ...
    """
    return flectra.tools.table_exists(cr, 'ir_module_module')
...
def initialize(cr):
    ...
    """
    f = flectra.modules.get_module_resource('base', 'base.sql')
    if not f:
        m = "File not found: 'base.sql' (provided by module 'base')."
        _logger.critical(m)
        raise IOError(m)
    base_sql_file = flectra.tools.misc.file_open(f)
    try:
    ...
    for i in flectra.modules.get_modules():
        ...
        info = flectra.modules.load_information_from_description_file(i)
